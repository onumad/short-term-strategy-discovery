...
class Phase6FailureSynthesisTests(unittest.TestCase):
    ...
    def test_render_phase6_failure_synthesis_captures_counts_best_candidates_and_recommendation(self) -> None:
        ...
        self.assertIn("Source Artifacts And Repro Commands", report)
        self.assertIn("outputs/phase6a_expansion_results.csv", report)
        self.assertIn("outputs/phase6b_ambiguity_reduction_results.csv", report)
        self.assertIn("./.venv/Scripts/python.exe scripts/run_phase6_failure_synthesis.py", report)
        self.assertIn("./.venv/Scripts/python.exe -m unittest discover -s tests -v", report)
