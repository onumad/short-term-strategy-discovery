...
from short_term_edge.portfolio_audit_c import (  # noqa: E402
    ...
    write_portfolio_audit_c_outputs,
)

EXPERIMENT_NAME = "portfolio_audit_c"
RUN_COMMAND = "EXPERIMENT_RUN_ID=portfolio-audit-c-r1 ./.venv/Scripts/python.exe scripts/run_portfolio_audit_c.py"
GUARDRAILS = [
...
def main() -> None:
    ...
    report_path = PROJECT_ROOT / "reports" / "portfolio_audit_c_report.md"
    result = run_portfolio_audit_c(output_dir)
    paths = write_portfolio_audit_c_outputs(result, output_dir, report_path)
    run_paths = prepare_experiment_run(PROJECT_ROOT, EXPERIMENT_NAME, os.environ.get("EXPERIMENT_RUN_ID", "portfolio-audit-c-r1"))
    ...
    run_paths.report_path.write_text(render_portfolio_audit_c_report(result, run_paths.report_path), encoding="utf-8")
    manifest = write_experiment_manifest(
        ...
        config={
            ...
            "official_gates_unchanged": True,
            "paper_trading_approved": False,
            "input_source": "playbook module registry, existing phase outputs, and Portfolio Audit B outputs only",
    ...
    print("Portfolio Audit C complete.")
