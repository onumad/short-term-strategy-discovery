...
EXPERIMENT_NAME = "phase12a_opening_drive_first_pullback"
RUN_COMMAND = "EXPERIMENT_RUN_ID=phase12a-r1-smoke ./.venv/Scripts/python.exe scripts/run_phase12a_opening_drive_first_pullback.py"
GUARDRAILS = [
    ...
    "MNQ-only 48-spec RTH opening-drive first-pullback continuation test; no overnight-derived filters",
]
...
def main() -> None:
    ...
    config = Phase12AConfig(max_specs=int(os.environ.get("PHASE12A_MAX_SPECS", "48")))
    specs = build_phase12a_specs(config)
    bars = _load_recent_mnq_bars(config.recent_sessions)
    result = run_phase12a_retest(bars, config)
    recommendation = make_phase12a_recommendation(result)
    run_paths = prepare_experiment_run(PROJECT_ROOT, EXPERIMENT_NAME, run_id=os.environ.get("EXPERIMENT_RUN_ID"))
    paths = {
        "candidate_results": output_dir / "phase12a_candidate_results.csv",
        "trade_logs": output_dir / "phase12a_trade_logs.csv",
        "walk_forward_folds": output_dir / "phase12a_walk_forward_folds.csv",
        "daily_pnl": output_dir / "phase12a_daily_pnl.csv",
        "concentration_diagnostics": output_dir / "phase12a_concentration_diagnostics.csv",
        "od_window_summary": output_dir / "phase12a_od_window_summary.csv",
        "side_summary": output_dir / "phase12a_side_summary.csv",
        "pullback_anchor_summary": output_dir / "phase12a_pullback_anchor_summary.csv",
        "confirmation_summary": output_dir / "phase12a_confirmation_summary.csv",
        "exit_variant_summary": output_dir / "phase12a_exit_variant_summary.csv",
        "exit_reason_summary": output_dir / "phase12a_exit_reason_summary.csv",
        "opening_drive_width_summary": output_dir / "phase12a_opening_drive_width_summary.csv",
        "opening_drive_close_position_summary": output_dir / "phase12a_opening_drive_close_position_summary.csv",
        "extension_distance_summary": output_dir / "phase12a_extension_distance_summary.csv",
        "pullback_depth_summary": output_dir / "phase12a_pullback_depth_summary.csv",
        "mfe_mae_summary": output_dir / "phase12a_mfe_mae_summary.csv",
        "invalid_risk_summary": output_dir / "phase12a_invalid_risk_summary.csv",
        "strategy_specs": output_dir / "phase12a_strategy_specs.json",
        "recommendation": output_dir / "phase12a_next_action_recommendation.json",
    }
    report_path = report_dir / "phase12a_opening_drive_first_pullback_report.md"
    ...
    report = render_phase12a_report(result, recommendation, report_path)
    ...
    run_paths.specs_path.write_text(serialize_phase12a_specs(specs), encoding="utf-8")
    run_paths.report_path.write_text(render_phase12a_report(result, recommendation, run_paths.report_path), encoding="utf-8")
    ...
    print("Phase 12A opening-drive first pullback continuation complete.")
    ...
    if top is not None:
        print(f"Top candidate: {top['candidate_id']} ({top['phase12a_label']} / {top['research_axis_status']})")
