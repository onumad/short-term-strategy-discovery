...
from short_term_edge.portfolio_audit_a import (  # noqa: E402
    ...
    write_portfolio_audit_outputs,
)

EXPERIMENT_NAME = "portfolio_audit_a"
RUN_COMMAND = "EXPERIMENT_RUN_ID=portfolio-audit-a-r1 ./.venv/Scripts/python.exe scripts/run_portfolio_audit_a.py"
GUARDRAILS = [
...
def main() -> None:
    ...
    report_path = PROJECT_ROOT / "reports" / "portfolio_audit_a_report.md"
    result = run_portfolio_audit_a(output_dir)
    paths = write_portfolio_audit_outputs(result, output_dir, report_path)
    run_paths = prepare_experiment_run(PROJECT_ROOT, EXPERIMENT_NAME, os.environ.get("EXPERIMENT_RUN_ID", "portfolio-audit-a-r1"))
    ...
    run_paths.report_path.write_text(render_portfolio_audit_report(result, run_paths.report_path), encoding="utf-8")
    manifest = write_experiment_manifest(
        ...
        config={
            ...
            "input_source": "research registry and existing phase outputs only",
    ...
    print("Portfolio Audit A complete.")
    print(f"Selected signals: {len(result['signal_selection'])}")
