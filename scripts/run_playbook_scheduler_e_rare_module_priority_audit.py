...
from short_term_edge.playbook_scheduler_e_rare_module_priority_audit import (  # noqa: E402
    ...
    best_scheduler_e_result,
    loaded_input_names,
    render_playbook_scheduler_e_report,
    run_playbook_scheduler_e_rare_module_priority_audit,
    write_playbook_scheduler_e_outputs,
)

EXPERIMENT_NAME = "playbook_scheduler_e"
RUN_COMMAND = "EXPERIMENT_RUN_ID=playbook-scheduler-e-r1 ./.venv/Scripts/python.exe scripts/run_playbook_scheduler_e_rare_module_priority_audit.py"
GUARDRAILS = [
    "research/simulation only",
    "rare-module scheduler audit only",
    "existing local outputs only",
...
def main() -> None:
    ...
    report_path = PROJECT_ROOT / "reports" / "playbook_scheduler_e_rare_module_priority_audit_report.md"
    result = run_playbook_scheduler_e_rare_module_priority_audit(output_dir)
    paths = write_playbook_scheduler_e_outputs(result, output_dir, report_path)

    run_paths = prepare_experiment_run(PROJECT_ROOT, EXPERIMENT_NAME, os.environ.get("EXPERIMENT_RUN_ID", "playbook-scheduler-e-r1"))
    for key in ("policy_results", "daily_pnl", "walk_forward_folds", "concentration", "overlap_summary", "rare_module_acceptance_summary", "rare_module_impact", "weak_regime_impact"):
        value = result[key]
    ...
    specs = {
        ...
        "rare_priority_policies": list(POLICIES),
        "modes": list(MODES),
        "selected_signal_keys": result["selected_signal_keys"],
        "rare_signal_keys": result["rare_signal_keys"],
        "phase16a_rare_signal_keys": result["phase16a_rare_signal_keys"],
        "inputs_loaded": loaded_input_names(),
    }
    write_json_artifact(specs, run_paths.specs_path)
    run_paths.report_path.write_text(render_playbook_scheduler_e_report(result), encoding="utf-8")
    ...
    manifest = write_experiment_manifest(
        ...
        selected_specs_count=len(POLICIES) * len(MODES),
    ...
    print("Playbook Scheduler E rare-module priority audit complete.")
    print(f"Selected modules: {len(result['selected_signal_keys'])}")
    print(f"Rare modules: {len(result['rare_signal_keys'])}")
    print(f"Phase 16A rare modules: {len(result['phase16a_rare_signal_keys'])}")
    print(f"Policies: {', '.join(POLICIES)}")
    print(f"Rows: {len(result['policy_results'])}")
    print(f"Best Scheduler E result: {best.get('rare_priority_policy')} / {best.get('portfolio_mode')} net={best.get('net_pnl')} pos_folds={best.get('positive_wf_test_folds_pct')} label={best.get('scheduler_e_label')}")
