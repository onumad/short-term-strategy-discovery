...
from short_term_edge.phase15a_trend_power_continuation_scout import (  # noqa: E402
    ...
    run_phase15a_scout,
    serialize_phase15a_specs,
)

EXPERIMENT_NAME = "phase15a_trend_power_continuation_scout"
RUN_COMMAND = "EXPERIMENT_RUN_ID=phase15a-r1-smoke ./.venv/Scripts/python.exe scripts/run_phase15a_trend_power_continuation_scout.py"
GUARDRAILS = [
    ...
    "MNQ-only 48-spec trend-day, power-hour, and low-volatility expansion continuation scout; excludes MGC, prior-RTH high/low breakout, prior-RTH close/midpoint reaction, overnight levels, opening range fade, opening-drive first pullback, VWAP, and volatility-compression logic",
]
...
def main() -> None:
    ...
    config = Phase15AConfig(max_specs=int(os.environ.get("PHASE15A_MAX_SPECS", "48")))
    ...
    playbook_daily = pd.read_csv(output_dir / "portfolio_audit_c_portfolio_daily_pnl.csv")
    gap_features = pd.read_csv(output_dir / "playbook_gap_audit_a_market_day_features.csv")
    result = run_phase15a_scout(bars, registry_matrix, playbook_daily, gap_features, config)
    recommendation = make_phase15a_recommendation(result)
    run_paths = prepare_experiment_run(PROJECT_ROOT, EXPERIMENT_NAME, run_id=os.environ.get("EXPERIMENT_RUN_ID"))
    paths = {
        "candidate_results": output_dir / "phase15a_candidate_results.csv",
        "trade_logs": output_dir / "phase15a_trade_logs.csv",
        "daily_pnl": output_dir / "phase15a_daily_pnl.csv",
        "walk_forward_folds": output_dir / "phase15a_walk_forward_folds.csv",
        "concentration_diagnostics": output_dir / "phase15a_concentration_diagnostics.csv",
        "family_summary": output_dir / "phase15a_family_summary.csv",
        "side_summary": output_dir / "phase15a_side_summary.csv",
        "trigger_model_summary": output_dir / "phase15a_trigger_model_summary.csv",
        "confirmation_summary": output_dir / "phase15a_confirmation_summary.csv",
        "exit_variant_summary": output_dir / "phase15a_exit_variant_summary.csv",
        "correlation_to_registry": output_dir / "phase15a_correlation_to_registry.csv",
        "correlation_to_playbook": output_dir / "phase15a_correlation_to_playbook.csv",
        "gap_coverage_summary": output_dir / "phase15a_gap_coverage_summary.csv",
        "strategy_specs": output_dir / "phase15a_strategy_specs.json",
        "recommendation": output_dir / "phase15a_next_action_recommendation.json",
    }
    report_path = report_dir / "phase15a_trend_power_continuation_scout_report.md"
    ...
    report_path.write_text(render_phase15a_report(result, recommendation, report_path), encoding="utf-8")
    ...
    run_paths.specs_path.write_text(serialize_phase15a_specs(specs), encoding="utf-8")
    run_paths.report_path.write_text(render_phase15a_report(result, recommendation, run_paths.report_path), encoding="utf-8")
    ...
    print("Phase 15A trend/power continuation scout complete.")
    ...
    if top is not None:
        print(f"Top candidate: {top['candidate_id']} ({top['phase15a_label']})")
