...
MODULE_FAMILIES = (
    "high_vol_mixed_extreme_fade",
    "high_vol_mixed_midpoint_reclaim",
    "high_vol_mixed_late_resolution_breakout",
)
SIDES = ("long", "short")
REGIME_VARIANTS = ("broad_high_vol_mixed", "strict_high_vol_mixed")
...
FAMILY_META = {
    "high_vol_mixed_extreme_fade": {"trade_start": "12:00", "trade_end": "15:30", "target_type": "morning_midpoint"},
    "high_vol_mixed_midpoint_reclaim": {"trade_start": "12:00", "trade_end": "15:30", "target_type": "morning_opposite_extreme"},
    "high_vol_mixed_late_resolution_breakout": {"trade_start": "13:00", "trade_end": "15:30", "target_type": "fixed_1_5r"},
}
TARGET_GAP_FLAGS = (
    ...
    "prior_level_interaction_days",
)


@dataclass(frozen=True)
class Phase16AConfig:
    max_specs: int = 48
...
class Phase16ASpec:
    ...
    def candidate_id(self) -> str:
        return f"MNQ_16a_{self.module_family}_{self.side}_{self.regime_variant}_{self.confirmation_model}_{self.exit_variant}"
    ...
    def to_dict(self) -> dict[str, Any]:
        ...
        return {
            ...
            "regime_build_end": "11:30",
            "lunch_build_start": "11:30" if self.module_family == "high_vol_mixed_late_resolution_breakout" else None,
            "lunch_build_end": "13:00" if self.module_family == "high_vol_mixed_late_resolution_breakout" else None,
...
def build_phase16a_specs(config: Phase16AConfig = Phase16AConfig()) -> list[Phase16ASpec]:
    specs: list[Phase16ASpec] = []
            ...
            for regime in REGIME_VARIANTS:
                for confirm in CONFIRMATION_MODELS:
                    for exit_variant in EXIT_VARIANTS:
                        specs.append(Phase16ASpec(family, side, regime, confirm, exit_variant))
...
def compute_phase16a_frozen_levels(bars5: pd.DataFrame, config: Phase16AConfig = Phase16AConfig()) -> pd.DataFrame:
    ...
    for session, day in bars5.groupby("trading_session", sort=True):
        ...
        lunch = _between(day, "11:30", "13:00")
        row: dict[str, Any] = {
            "trading_session": session,
            "prior_morning_range_sessions_used": len(prior_morning_ranges),
            "morning_range_p70": _prior_percentile(prior_morning_ranges, 0.70) if len(prior_morning_ranges) >= config.min_prior_sessions_for_percentile else None,
            "morning_range_p80": _prior_percentile(prior_morning_ranges, 0.80) if len(prior_morning_ranges) >= config.min_prior_sessions_for_percentile else None,
        }
        if not morning.empty:
            ...
            post30 = _between(day, "10:00", "11:30")
            first_dir = _direction(float(first30.iloc[0]["open"]), float(first30.iloc[-1]["close"])) if not first30.empty else "flat"
            post_dir = _direction(float(post30.iloc[0]["open"]), float(post30.iloc[-1]["close"])) if not post30.empty else "flat"
            flip = first_dir != "flat" and post_dir != "flat" and first_dir != post_dir
            close_pos = safe_divide(mc - ml, mr)
            p70 = row["morning_range_p70"]; p80 = row["morning_range_p80"]
            broad = bool(p70 is not None and mr >= float(p70) and (0.25 <= close_pos <= 0.75 or flip))
            strict = bool(p80 is not None and mr >= float(p80) and 0.35 <= close_pos <= 0.65 and flip)
            row.update({
                "morning_high": mh,
                "morning_low": ml,
                "morning_midpoint": (mh + ml) / 2.0,
                "morning_open": mo,
                "morning_close": mc,
                "morning_range": mr,
                "morning_close_position": close_pos,
                "first_30m_direction": first_dir,
                "post_30m_to_1130_direction": post_dir,
                "direction_flip_flag": flip,
                "broad_high_vol_mixed": broad,
                "strict_high_vol_mixed": strict,
                "morning_build_bar_count": len(morning),
            })
            prior_morning_ranges.append(mr)
        else:
            row.update({"broad_high_vol_mixed": False, "strict_high_vol_mixed": False, "morning_build_bar_count": 0})
        if not lunch.empty:
            lh = float(lunch["high"].max()); ll = float(lunch["low"].min())
            row.update({"lunch_high": lh, "lunch_low": ll, "lunch_midpoint": (lh + ll) / 2.0, "lunch_range": lh - ll, "lunch_build_bar_count": len(lunch)})
        else:
            row.update({"lunch_build_bar_count": 0})
        rows.append(row)
    return pd.DataFrame(rows)


def build_phase16a_feature_bars(bars: pd.DataFrame, config: Phase16AConfig = Phase16AConfig()) -> pd.DataFrame:
    bars5 = resample_rth_5m(bars)
    if bars5.empty:
        return pd.DataFrame()
    levels = compute_phase16a_frozen_levels(bars5, config)
    out = bars5.merge(levels, on="trading_session", how="left")
    return out.sort_values(["trading_session", "timestamp"]).reset_index(drop=True)


def generate_phase16a_signals(features: pd.DataFrame, spec: Phase16ASpec) -> list[dict[str, Any]]:
    if features.empty:
        return []
...
def _signal_condition(row: pd.Series, spec: Phase16ASpec) -> bool:
    ...
    if spec.module_family == "high_vol_mixed_extreme_fade":
        return (low < ml and close > ml) if spec.side == "long" else (high > mh and close < mh)
    if spec.module_family == "high_vol_mixed_midpoint_reclaim":
        return (low < mid and close > mid) if spec.side == "long" else (high > mid and close < mid)
    if pd.isna(row.get("lunch_high")) or pd.isna(row.get("lunch_low")):
        return False
    return close > float(row["lunch_high"]) if spec.side == "long" else close < float(row["lunch_low"])


def _confirm_bar_ok(row: pd.Series, spec: Phase16ASpec) -> bool:
    if spec.module_family == "high_vol_mixed_extreme_fade":
    ...
    if spec.module_family == "high_vol_mixed_midpoint_reclaim":
        ...
        return float(row["close"]) > level if spec.side == "long" else float(row["close"]) < level
    level = float(row["lunch_high"] if spec.side == "long" else row["lunch_low"])
    return float(row["close"]) > level if spec.side == "long" else float(row["close"]) < level


def _signal_dict(spec: Phase16ASpec, trigger_row: pd.Series, signal_row: pd.Series, entry: pd.Series, confirmation_time: Any, pullback_low: float, pullback_high: float) -> dict[str, Any]:
    return {
...
def simulate_phase16a_trades(features: pd.DataFrame, signals: list[dict[str, Any]], spec: Phase16ASpec) -> tuple[pd.DataFrame, int]:
...
def _simulate_one(day: pd.DataFrame, entry_pos: int, signal: dict[str, Any], spec: Phase16ASpec, inst) -> dict[str, Any] | None:
...
def _target_price(signal: dict[str, Any], spec: Phase16ASpec, entry_price: float, risk: float) -> float:
    if spec.module_family == "high_vol_mixed_extreme_fade":
...
def run_phase16a_scout(bars: pd.DataFrame, registry_matrix: pd.DataFrame, playbook_daily: pd.DataFrame, scheduler_daily: pd.DataFrame, gap_features: pd.DataFrame, validation_policy: dict[str, Any] | None = None, config: Phase16AConfig = Phase16AConfig()) -> dict[str, pd.DataFrame]:
    config = _config_with_policy(config, validation_policy or {})
    specs = build_phase16a_specs(config)
    ...
    features = build_phase16a_feature_bars(scoped, config) if specs else pd.DataFrame()
    ...
    candidates = pd.DataFrame(rows).sort_values(["phase16a_score", "stress_pnl"], ascending=[False, False]).reset_index(drop=True)
    candidates.insert(0, "phase16a_rank", range(1, len(candidates) + 1))
    return {
...
def _candidate_row(spec: Phase16ASpec, trades: pd.DataFrame, invalid: int, signals: list[dict[str, Any]], sessions: list[str], split_map: dict[Any, str], registry_matrix: pd.DataFrame, playbook_daily: pd.DataFrame, scheduler_daily: pd.DataFrame, gap_sets: dict[str, set[str]], current_covered: set[str], folds: pd.DataFrame, adequacy: pd.DataFrame, config: Phase16AConfig) -> dict[str, Any]:
    row = spec.to_dict()
    ...
    target_gap = set().union(gap_sets.get("high_vol_mixed_power_expand_days", set()), gap_sets.get("high_vol_mixed_no_power_expand_days", set()), gap_sets.get("prior_level_interaction_days", set()))
    ...
    row["phase16a_score"] = round(float(row.get("stress_pnl", 0)) + float(row.get("walk_forward_stress_pnl", 0)) + 25 * float(row.get("incremental_gap_days_covered", 0)) - abs(float(row.get("max_drawdown", 0))) - 1000 * max(float(row.get("average_correlation_to_registry", 1)) - config.registry_avg_corr_limit, 0), 4)
    return row
...
def make_phase16a_recommendation(result: dict[str, pd.DataFrame]) -> dict[str, Any]:
    c = result["candidate_results"]
    base = {"official_gates_changed": False, "paper_trading_approved": False, "live_trading_approved": False, "candidates_promoted": False}
    if c.empty:
        return {**base, "next_action": "phase16b_target_next_gap_or_manual_review", "rationale": "No Phase 16A candidates were produced."}
    paper = c[c["phase16a_label"].eq("phase16a_candidate_for_paper_review")]
    if not paper.empty:
        return {**base, "next_action": "phase16a_review_packet_only", "rationale": "A candidate reached review-packet diagnostics only; paper trading remains false.", "top_candidate": paper.iloc[0].to_dict()}
    if (c["fold_adequacy_status"].astype(str).eq("low_activity_not_fully_interpretable")).all():
        return {**base, "next_action": "rare_module_validation_track_review", "rationale": "Fold adequacy is sparse under standardized module-level policy.", "top_candidate": c.iloc[0].to_dict()}
    uncorr = c[c["phase16a_label"].eq("phase16a_positive_uncorrelated_research_signal")]
    if not uncorr.empty:
        return {**base, "next_action": "add_to_registry_and_run_portfolio_audit_e", "rationale": "A positive high-vol mixed axis was uncorrelated to existing registry/playbook diagnostics.", "top_candidate": uncorr.iloc[0].to_dict()}
    narrow = c[(c["incremental_gap_days_covered"] > 0) & (c["stress_pnl"] > 0)]
    if not narrow.empty:
        return {**base, "next_action": "phase16b_targeted_high_vol_mixed_diagnostic", "rationale": "A candidate improved gap coverage but missed at least one tradability gate.", "top_candidate": narrow.iloc[0].to_dict()}
    positive = c[(c["stress_pnl"] > 0) & (c["validation_pnl"] > 0) & (c["holdout_pnl"] > 0)]
    if not positive.empty:
        return {**base, "next_action": "add_to_registry_as_parked_high_vol_mixed_signal", "rationale": "Positive high-vol mixed axes remained correlated, concentrated, or unstable.", "top_candidate": positive.iloc[0].to_dict()}
    return {**base, "next_action": "phase16b_target_next_gap_or_manual_review", "rationale": "No positive specialized/uncorrelated high-vol mixed axis survived stress/validation/holdout diagnostics."}


def render_phase16a_report(result: dict[str, pd.DataFrame], recommendation: dict[str, Any], report_path: Path) -> str:
    c = result["candidate_results"]
    counts = c["phase16a_label"].value_counts().to_dict() if not c.empty else {}
    lines = ["# Phase 16A — High-Vol Mixed Regime Module Scout", "", RESEARCH_ONLY_GUARDRAIL, "", "Bounded 48-spec MNQ-only high-volatility mixed-regime scout. No MGC, no prior-RTH high/low breakout, no prior-RTH close/midpoint reaction, no Phase 15A trend/power continuation, no overnight levels, no opening range/opening-drive/VWAP/volatility-compression logic, no official gate changes, no promotions, and no paper trading approval.", "", "## Summary", "", f"- Specs evaluated: `{len(c)}`", f"- Trade rows: `{len(result['trade_logs'])}`", f"- Label counts: `{counts}`", f"- Next action: `{recommendation.get('next_action')}`", f"- Rationale: {recommendation.get('rationale')}", "- Paper trading approved: `false`", "", "## Top Candidates", "", "| Candidate | Label | Net | Stress | Val | Holdout | WF Stress | Avg reg corr | Avg playbook corr | Gap days | Incremental gap days | Fold adequacy | Reasons |", "| --- | --- | ---: | ---: | ---: | ---: | ---: | ---: | ---: | ---: | ---: | --- | --- |"]
    for _, r in c.head(12).iterrows():
        lines.append(f"| `{r['candidate_id']}` | {r['phase16a_label']} | {float(r['net_pnl']):.2f} | {float(r['stress_pnl']):.2f} | {float(r['validation_pnl']):.2f} | {float(r['holdout_pnl']):.2f} | {float(r['walk_forward_stress_pnl']):.2f} | {float(r['average_correlation_to_registry']):.3f} | {float(r['average_correlation_to_playbook']):.3f} | {int(r['gap_days_covered'])} | {int(r['incremental_gap_days_covered'])} | {r['fold_adequacy_status']} | {r['reject_reasons']} |")
    lines += ["", "## Fold Views", "", "Required fold views reported: existing_project_folds, half_year_folds, rolling_6_month_test_folds, quarterly_folds. Alternative folds are diagnostic companions only; official gates unchanged.", "", "## Outputs", "", f"- `{report_path.as_posix()}`", "- `outputs/phase16a_candidate_results.csv`", "- `outputs/phase16a_trade_logs.csv`", "- `outputs/phase16a_gap_coverage_summary.csv`", "- `outputs/phase16a_fold_view_summary.csv`", "- `outputs/phase16a_module_fold_adequacy.csv`", "- `outputs/phase16a_next_action_recommendation.json`"]
    return "\n".join(lines) + "\n"


def _label(r: dict[str, Any], c: Phase16AConfig) -> str:
    ...
    if adequate and econ and fold_ok and conc_ok:
        return "phase16a_candidate_for_paper_review"
    if econ and corr_ok:
        return "phase16a_positive_uncorrelated_research_signal"
    if econ:
        return "phase16a_positive_specialized_research_signal"
    if r.get("stress_pnl", 0) > 0 and r.get("gap_days_covered", 0) > 0:
        return "phase16a_watchlist_needs_more_history"
    if r.get("net_pnl", 0) <= 0 or r.get("stress_pnl", 0) <= 0:
        return "phase16a_rejected_negative_stress"
    if r.get("validation_pnl", 0) <= 0:
        return "phase16a_rejected_negative_validation"
    if r.get("holdout_pnl", 0) <= 0:
        return "phase16a_rejected_negative_holdout"
    if not corr_ok:
        return "phase16a_rejected_high_correlation"
    if not adequate:
        return "phase16a_rejected_low_activity"
    if not fold_ok:
        return "phase16a_rejected_fold_instability"
    if not conc_ok:
        return "phase16a_rejected_concentration"
    return "phase16a_rejected_fold_instability"


def _signal_evidence(r: dict[str, Any]) -> str:
    label = str(r.get("phase16a_label"))
    if label in {"phase16a_candidate_for_paper_review", "phase16a_positive_uncorrelated_research_signal", "phase16a_positive_specialized_research_signal"}:
        return "positive_research_signal"
    if label == "phase16a_watchlist_needs_more_history" or r.get("stress_pnl", 0) > 0:
...
def _tradability(r: dict[str, Any], c: Phase16AConfig) -> str:
    label = str(r.get("phase16a_label"))
    if label == "phase16a_candidate_for_paper_review":
        return "review_packet_candidate"
    if label == "phase16a_watchlist_needs_more_history":
...
def _research_track(r: dict[str, Any]) -> str:
    if r.get("phase16a_label") in {"phase16a_candidate_for_paper_review", "phase16a_watchlist_needs_more_history", "phase16a_positive_uncorrelated_research_signal"}:
...
def _portfolio_role(r: dict[str, Any]) -> str:
    if r.get("phase16a_label") == "phase16a_positive_uncorrelated_research_signal":
        return "diversifier_module"
    if r.get("phase16a_label") == "phase16a_candidate_for_paper_review":
...
def _rolling_fold_rows(trades: pd.DataFrame, spec: Phase16ASpec, sessions: list[str], window: int, test_offset: int, step: int, view: str) -> list[dict[str, Any]]:
    rows = []
    start = 0; fold = 1
...
def _block_fold_rows(trades: pd.DataFrame, spec: Phase16ASpec, sessions: list[str], block: int, view: str) -> list[dict[str, Any]]:
    rows = []
    fold = 1
...
def _fold_row(spec: Phase16ASpec, view: str, fold: int, test_sessions: list[str], seg: pd.DataFrame) -> dict[str, Any]:
...
def _fold_adequacy_rows(folds: pd.DataFrame, spec: Phase16ASpec, c: Phase16AConfig) -> pd.DataFrame:
    if folds.empty:
        return pd.DataFrame()
    out = folds.copy()
...
def _gap_session_sets(gap_features: pd.DataFrame, playbook_daily: pd.DataFrame, scheduler_daily: pd.DataFrame) -> dict[str, set[str]]:
    ...
    prior = gf.get("prior_rth_high_low_interaction_flag", False).astype(bool) if "prior_rth_high_low_interaction_flag" in gf else pd.Series(False, index=gf.index)
    return {
        ...
        "prior_level_interaction_days": set(sessions[prior]),
    }
...
def _config_with_policy(config: Phase16AConfig, policy: dict[str, Any]) -> Phase16AConfig:
    defaults = policy.get("fold_adequacy_defaults", {}) if isinstance(policy, dict) else {}
    return Phase16AConfig(**{**config.__dict__, "module_fold_min_active_days": int(defaults.get("module_fold_min_active_days", config.module_fold_min_active_days)), "module_fold_min_trades": int(defaults.get("module_fold_min_trades", config.module_fold_min_trades))})
...
def serialize_phase16a_specs(specs: list[Phase16ASpec]) -> str:
